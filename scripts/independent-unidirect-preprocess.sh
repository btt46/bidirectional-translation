...
for SET in $DATA_NAME; do
    subword-nmt apply-bpe -c $BPE_MODEL/code.${BPESIZE}.bpe --vocabulary $BPE_MODEL/train.${SRC}.vocab --vocabulary-threshold 50 < ${PROCESSED_DATA}/${SET}.${SRC} > $BPE_DATA/${SET}.${SRC}
    subword-nmt apply-bpe -c $BPE_MODEL/code.${BPESIZE}.bpe --vocabulary $BPE_MODEL/train.${TGT}.vocab --vocabulary-threshold 50 < ${PROCESSED_DATA}/${SET}.${TGT} > $BPE_DATA/${SET}.${TGT}
done
